...
function initWidgetView(_thisWidget) {
  ...
  $('#btn_measure_length_td').bind('click', () => {
    // 用户首次使用时，提醒一次
  ...
  $('#btn_measure_area_td').bind('click', () => {
    $('#lbl_measure_result').html('');
    $('#measure_danwei').show();
    $('#measure_length_danwei').hide();
    $('#measure_area_danwei').show();
  ...
  $('#btn_measure_section').bind('click', () => {
    // 用户首次使用时，提醒一次
  ...
  $('#btn_measure_supHeight').bind('click', () => {
    $('#lbl_measure_result').html('');
    $('#measure_danwei').show();
    $('#measure_area_danwei').hide();
    $('#measure_length_danwei').show();
  ...
  // 更换单位
  $('#measure_length_danwei').change((e) => {
    const danwei = $('#measure_length_danwei').val();
    thisWidget.updateUnit(thisType, danwei);

    if (lastVal > 0) {
      const valstr = thisWidget.formatLength(lastVal, danwei);
      onMeasureChange(valstr);
    }
  });
  $('#measure_area_danwei').change((e) => {
    const danwei = $('#measure_area_danwei').val();
    thisWidget.updateUnit(thisType, danwei);

    if (lastVal > 0) {
      const valstr = thisWidget.formatArea(lastVal, danwei);
      onMeasureChange(valstr);
    }
  });
