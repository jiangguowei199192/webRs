...
mars3d.widget.bindClass(mars3d.widget.BaseWidget.extend({
  ...
  addOverlay(item) {
    ...
    if (!item.id) { item.id = viewer.mars.getNextId(); }
    if (!item.pid) { item.pid = -1; }

    // 计算层次顺序
    let order = Number(item.order);
    if (isNaN(order)) { order = this._layers.length + 1; }
    item.order = order;
  ...
  getLayers() {
    if (this._layers == null) {
      const layers = [];
      const basemapsCfg = this.hasManagerBaseMaps ? this.viewer.mars.config.basemaps : [];
      const operationallayersCfg = this.viewer.mars.config.operationallayers;
  ...
  centerAt(item) {
    const model = this.getLayar(item);
