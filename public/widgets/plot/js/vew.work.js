...
function initWidgetView(_thisWidget) {
  ...
  $('#btn_plot_end').click((e) => {
    thisWidget.endDraw();
  });

  $('#btn_plot_socket').click((e) => {
...
var plotFile = {
  initEvent() {
    ...
    let isClearForOpenFile;
    $('#btn_plot_openfile').click(() => {
      isClearForOpenFile = true;
      $('#input_plot_file').click();
    });

    $('#btn_plot_openfile2').click(() => {
      isClearForOpenFile = false;
      $('#input_plot_file').click();
    });

    $('#btn_plot_savefile').click(() => {
      const data = thisWidget.getGeoJson();
      if (data == null || data == '') {
...
var plotlist = {
  ...
  bindSelList() {
    const that = this;
    const $sel_plot_list = $('#sel_plot_list');
    $.getJSON('config/plotlist.json', (plotlist) => {
      let inhtml = '';
      let defval;
      let count = 0;
      for (const i in plotlist) {
  ...
  showPlotList(list) {
    ...
    const serverURL = thisWidget.getServerURL();
    const serverURL_gltf = thisWidget.getGltfServerURL();
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      if (item.style && item.style.modelUrl) {
        if (item.style.modelUrl.startsWith('$plot$')) {
          ...
          item.style.modelUrl = item.style.modelUrl.replace('$serverURL_gltf$', serverURL_gltf);
        } else if (item.style.modelUrl.startsWith('$serverURL$')) {
          item.style.modelUrl = item.style.modelUrl.replace('$serverURL$', serverURL);
        }
      }

      const defStyle = thisWidget.getDefStyle(item.edittype || item.type);
      ...
      if (image) {
        if (image.startsWith('http')) {
          ...
          image = image.replace('$serverURL_gltf$', serverURL_gltf);
  ...
  startPlot(idx, li) {
    ...
    if (this._lastLi) { this._lastLi.removeClass('markon'); }
    this._lastLi = _thisli;

    const item = haoutil.system.clone(this._listData[idx] || {});
    delete item.image;
    ...
    if (defStyle) {
      ...
      for (const i in defStyle) {
        if (item.style[i] == null) { item.style[i] = defStyle[i]; }
      }
    }
...
var tableWork = {
  ...
  initEvent() {
    ...
    $table.bootstrapTable({
      ...
      onClickRow(rowData, $element, field) {
  ...
  loadData(arr) {
    const positon = this.$table.bootstrapTable('getScrollPosition');
