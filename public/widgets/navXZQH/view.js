...
function centerAtRegion(dmnm, dmmc) {
    ...
    if (dmnm.substring(2) == "0000") {//省
        jsonurl = "/geojson/xzqh/sheng/china.json";
        dmxh = dmnm.substring(0, 2);
    }
    else if (dmnm.substring(4) == "00") {//市
        jsonurl = "/geojson/xzqh/shi/" + dmnm.substring(0, 2) + ".json";
        dmxh = dmnm.substring(0, 4);
    }
    else {//县区
        jsonurl = "/geojson/xzqh/xian/" + dmnm.substring(0, 4) + "00.json";
        dmxh = dmnm;
        $("#con_wdx_1").hide();
    }


    $.getJSON(serverURL_file + jsonurl, function (geojson) {
